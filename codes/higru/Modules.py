...
class UttEncoder_basic(nn.Module):
	def __init__(self, d_word_vec, d_h1, type):
		super(UttEncoder_basic, self).__init__()
...
class combo_bin(nn.Module):
	def __init__(self, d_word_vec, d_h1, d_h2, d_fc, emodict, worddict, embedding, type='higru', bert_flag=False, don_model=0, trainable= False, feature_dim= 0, long_bert = 0):
		...
		if self.bidirectional==False:
			self.d_input= d_h2
			if self.model == 'higru-f' or self.model == 'combo-f_bin':
				self.d_input = d_h2 + d_h1
...
class combo_multi(nn.Module):
	def __init__(self, d_word_vec, d_h1, d_h2, d_fc, emodict, worddict, embedding, type='higru', bert_flag=False, don_model=0, trainable= False, feature_dim= 0, long_bert = 0):
		...
		if self.bidirectional==False:
			self.d_input= d_h2
			if self.model == 'higru-f' or self.model == 'combo-f_multi':
				self.d_input = d_h2 + d_h1
...
class BERT_HiGRU_basic(nn.Module):
	def __init__(self, d_word_vec, d_h1, d_h2, d_fc, emodict, worddict, embedding, type='higru', bert_flag=False, don_model=0, trainable= False, feature_dim= 0, long_bert = 0):
		super(BERT_HiGRU_basic, self).__init__()
		self.model = type
		self.max_length = worddict.max_length
		self.max_dialog = worddict.max_dialog
		self.d_h2 = d_h2
		...
		if self.bidirectional==False:
			...
			if self.model == 'higru-f':
				self.d_input = d_h2 + d_h1
			if self.model == 'higru-sf' or self.model == 'bert-higru-basic':
				self.d_input = 2 * d_h2 + d_h1
...
class BERT_HiGRU_sent_conn_mask_turn(nn.Module):
	def __init__(self, d_word_vec, d_h1, d_h2, d_fc, emodict, worddict, embedding, type='higru', bert_flag=False, don_model=0, trainable= False, feature_dim= 0, long_bert = 0):
		super(BERT_HiGRU_sent_conn_mask_turn, self).__init__()
		self.model = type
		self.max_length = worddict.max_length
		self.max_dialog = worddict.max_dialog
		self.d_h2 = d_h2
		...
		if self.bidirectional==False:
			...
			if self.model == 'higru-f':
				self.d_input = d_h2 + d_h1
			if self.model == 'higru-sf' or self.model == 'higru-sent-conn-mask-turn':
				self.d_input = 2 * d_h2 + d_h1
		...
		self.enc_dim = 24
		self.classifier = nn.Sequential(
			nn.Linear(d_h2 + feature_dim + self.num_classes + self.enc_dim, d_fc),
			nn.Dropout(0.5),
			nn.Linear(d_fc, self.num_classes)
		)
		...
		self.num_embedding = nn.Embedding(15, 24)
	...
	def forward(self, sents, lens, addn_feats, mask):
		...
		zeros_org = torch.zeros((1, self.num_classes)).cuda()
		...
		for i in range(output1.shape[0]):
			vec_here = output1[i,:]
			turn_enc = self.num_embedding(torch.tensor(turn_count).cuda())
			# pu.db
			vec_here = torch.cat([vec_here.unsqueeze(0), context, turn_enc.unsqueeze(0)], dim = -1)
...
class BERT_HiGRU_sent_conn_mask_turn_noenc(nn.Module):
	def __init__(self, d_word_vec, d_h1, d_h2, d_fc, emodict, worddict, embedding, type='higru', bert_flag=False, don_model=0, trainable= False, feature_dim= 0, long_bert = 0):
		super(BERT_HiGRU_sent_conn_mask_turn_noenc, self).__init__()
		self.model = type
		self.max_length = worddict.max_length
		self.max_dialog = worddict.max_dialog
		self.d_h2 = d_h2
		...
		if self.bidirectional==False:
			...
			if self.model == 'higru-f':
				self.d_input = d_h2 + d_h1
			if self.model == 'higru-sf' or self.model == 'higru-sent-conn-mask-turn-noenc':
				self.d_input = 2 * d_h2 + d_h1
...
class BERT_HiGRU_sent_conn_mask_interturn(nn.Module):
	def __init__(self, d_word_vec, d_h1, d_h2, d_fc, emodict, worddict, embedding, type='higru', bert_flag=False, don_model=0, trainable= False, feature_dim= 0, long_bert = 0):
		super(BERT_HiGRU_sent_conn_mask_interturn, self).__init__()
		self.model = type
		self.max_length = worddict.max_length
		self.max_dialog = worddict.max_dialog
		self.d_h2 = d_h2
		...
		if self.bidirectional==False:
			...
			if self.model == 'higru-f':
				self.d_input = d_h2 + d_h1
			if self.model == 'higru-sf' or self.model == 'higru-sent-conn-mask-interturn':
				self.d_input = 2 * d_h2 + d_h1
...
class BERT_HiGRU_sent_conn_mask_mid(nn.Module):
	def __init__(self, d_word_vec, d_h1, d_h2, d_fc, emodict, worddict, embedding, type='higru', bert_flag=False, don_model=0, trainable= False, feature_dim= 0, long_bert = 0):
		super(BERT_HiGRU_sent_conn_mask_mid, self).__init__()
		self.model = type
		self.max_length = worddict.max_length
		self.max_dialog = worddict.max_dialog
		self.d_fc = d_fc
		...
		if self.bidirectional==False:
			...
			if self.model == 'higru-f':
				self.d_input = d_h2 + d_h1
			if self.model == 'higru-sf' or self.model == 'higru-sent-conn-mask-mid':
				self.d_input = 2 * d_h2 + d_h1
		...
		self.classifier_f = nn.Sequential(
			nn.Linear(d_h2 + feature_dim + d_fc, d_fc),
			nn.Dropout(0.5),
			nn.ReLU()
		)
	...
	def forward(self, sents, lens, addn_feats, mask):
		...
		context = torch.zeros((1, self.d_fc)).cuda()
		output2 = torch.zeros((output1.shape[0], self.d_fc)).cuda()


		for i in range(output1.shape[0]):
			vec_here = output1[i,:]
			vec_here = torch.cat([vec_here.unsqueeze(0), context], dim = -1)
			here = self.classifier_f(vec_here)
			output2[i, :] = here
			if mask[i] == 1:
				context = here

		output  = self.classifier_s(output2)
		log_pred_scores = F.log_softmax(output, dim=1)
		pred_scores = F.softmax(output, dim=1)
...
class BERT_HiGRU_mask_outcome(nn.Module):
	def __init__(self, d_word_vec, d_h1, d_h2, d_fc, emodict, worddict, embedding, type='higru', bert_flag=False, don_model=2, trainable= False, feature_dim= 0, long_bert = 0):
		super(BERT_HiGRU_mask_outcome, self).__init__()
		self.model = type
		self.max_length = worddict.max_length
		self.max_dialog = worddict.max_dialog
		self.d_h2 = d_h2

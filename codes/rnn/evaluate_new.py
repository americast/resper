...
def evaluate(result, summary = False, file= None, epochs=10):
    f= open(file.replace('test','rnn_res')+'_'+UNIT+'_'+RNN_TYPE+'_'+ATTN+'_'+str(HIDDEN_SIZE)+'.txt','a')
    f.write(str(epochs)+'\n')
    ...
    for _, y0,_, y1, _ in result: # actual value, prediction
        # print(y0,y1)
        tp[y0] += y0 == y1
        tpfn[y0] += 1
        tpfp[y1] += 1
    ...
    print("accuracy = %f" % accuracy_score(ytrue, ypred))

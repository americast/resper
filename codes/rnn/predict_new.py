...
def run_model(model, itw, itt, batch):
    ...
    for i in range(batch_size):
        ...
        face_prob= torch.exp(result)

        face = None

        if batch[i][-1]=='1':
            face   = face_prob*EE_weights
        else:
            face   = face_prob*ER_weights

        y = itt[face[i].argmax()]
        p = round(max(result[i]).item(), NUM_DIGITS)
        batch[i].append(y)
        batch[i].append(p)
        if VERBOSE:
            print(batch[i][1])
...
def predict(filename, model, cti, wti, itt, itw):
    ...
    for idx, line in enumerate(fo):
        ...
        # y = line[-1]
        # line = " ".join(line[0:-1])
        line, y, speaker = line.split("\t") if line.count("\t") else [line, None, None]
        # line = line.split()
        # y = line[-2]
        # # speaker = line[-1]
        # line2 = " ".join(line[0:-2])
        ...
        data.append([idx, line, xc, xw, y, speaker])
    fo.close()
    with torch.no_grad():
        model.eval()
